...
First, we will try to scrap GOOGLE. But you can produce XML files for further text mining using other search engines (YANDEX) and codes (Rcrawler). Also, it is possible to do everything in Python.
...
Let's create the vector of first 10 scraped web pages about **???????????? ?????????????????????????????? ??????????????**:
```{r}
links <- c(
